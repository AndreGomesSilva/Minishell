...
int	str_len_token(char *str, int type)
{
	...
	else if (type == REDIRECT_HERD)
	{
		...
		while (str[i] && (!is_args(&str[i]) || is_args(&str[i]) == VAR_EXPAND) && !is_space(str[i]))
			i++;
	return (i);
}
...
char	*create_args(t_cmd *cmd, char *input)
{
	...
	while (is_space(*input))
		input++;
	if (!is_cmd(input) && (is_args(input) == QUOTE
			|| is_args(input) == VAR_EXPAND || is_args(input) == NORM)
		|| is_args(input) == DOUBLE_QUOTE)
	{
		len = str_len_token(input, is_args(input));
		if (is_args(input) == DOUBLE_QUOTE || is_args(input) == QUOTE)
			create_arg_node(cmd, ft_substr(input, 1, len - 2));
		else
			create_arg_node(cmd, ft_substr(input, 0, len));
		last_arg = get_last_node_arg(cmd->list_args);
		last_arg->type = is_args(input);
	}
	else
	{
		create_arg_node(cmd, "");
		get_last_node_arg(cmd->list_args)->type = is_args(input);
		if (is_args(input) == REDIRECT_INPUT
			|| is_args(input) == REDIRECT_OUTPUT)
			len += 1;
		else
			len += 2;
	}
...
char	*create_cmd(t_control *control, char *actual)
{
	...
	{
		len = str_len_token(actual, is_args(actual));
